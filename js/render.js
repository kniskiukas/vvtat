    ...
    return `
      <button class="option-btn${saved === opt.value ? ' selected' : ''}"
        ...
        <span class="option-body">
          ...
          ${opt.uploadKey && opt.uploadSub
            ? `<span class="option-desc">${escHtml(opt.uploadSub)}</span>`
            : opt.desc ? `<span class="option-desc">${escHtml(opt.desc)}</span>` : ''}
          ${opt.hint ? `<span class="option-hint">→ ${escHtml(opt.hint)}</span>` : ''}
        </span>
      </button>`;
  }).join('');
  ...
  document.getElementById('content').innerHTML = `
    <div class="card ${_slideClass}">
      ...
      <div class="actions">
        ...
        <a href="#${idToSlug(step.next)}" class="btn-next">${escHtml(step.nextLabel || 'Tęsti →')}</a>
      </div>
    </div>`;
}
  ...
  document.getElementById('content').innerHTML = `
    <div class="card ${_slideClass}">
      ...
      <div class="summary-cta">
        <div class="actions">
          <button class="btn-back" onclick="goBack()">← Atgal</button>
          <a href="${VVTAT_URL}" target="_blank" rel="noopener" class="btn-next">Pateikti prašymą VVTAT →</a>
        </div>
